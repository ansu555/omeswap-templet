...
function isResearchQuery(query: string): boolean {
  const explicitTicker = extractTicker(query)
  const tickerOnlyText = query
    .trim()
    .toUpperCase()
    .replace(/^\$/, '')
    .replace(/[?!.,]+$/, '')
  ...
  return (
    tickerOnly ||
    RESEARCH_INTENT_RE.test(query) ||
    (explicitTicker !== null && QUESTION_RE.test(query))
  )
}
...
export async function POST(req: NextRequest) {
  ...
  if (!isResearchQuery(query)) {
    return new Response(
      JSON.stringify({
        error: 'Ask for market research or include a ticker to start an ATS run.',
      }),
      { status: 400, headers: { 'Content-Type': 'application/json' } },
    )
  }
