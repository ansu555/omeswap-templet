...
import { BaseNode } from "@/lib/agent-builder/nodes/BaseNode";
import { createNodeInstance } from "@/lib/agent-builder/nodes/registry";
import { WORKFLOW_TEMPLATES } from "@/lib/agent-builder/templates";
import type { LogEntry } from "@/types/agent-builder-canvas";
import type { BacktestSummary } from "@/lib/agent-builder/backtest/BacktestRunner";
...
export const useStore = create<BotStore>((set, get) => ({
  ...
  loadWorkflow: (id) => {
    ...
    for (const node of workflow.nodes) {
      ...
      try {
        const inst = createNodeInstance(type, node.id);
        if (workflow.configs[node.id])
          inst.setConfig(workflow.configs[node.id]);
  ...
  setBacktestConfig: (patch) =>
    set((s) => ({ backtestConfig: { ...s.backtestConfig, ...patch } })),
